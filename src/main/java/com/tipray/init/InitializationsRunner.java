	...
	public void init(){
		try {
			...
			if(GridProperties.IS_DB_RESET || !SpringBeanUtil.getCommonService().isTablesExist()){
				execute(Initialization.MODE_CREATE);
			}else{
				execute(Initialization.MODE_UPDATE);
			}
	...
	public void execute(String mode) throws Exception {
		...
		for (Initialization init : inits) {
			...
			try {
				...
				init.execute(mode);
				logger.info("==>InitializationsRunner execute [{}] end,cost [{}]ms" ,init.toString(), System.currentTimeMillis() - startTime);
			} catch (Exception e) {
...
// 		addInitialization(new PermissionXmlInit());
	...
	private void addInitialization(Initialization init) {
