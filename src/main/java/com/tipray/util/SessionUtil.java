...
public class SessionUtil {
	...
	private static final Long SESSION_TIMEOUT_MILLISECOND = 30 * 60 * 1000L;
	 ...
	 * 获取sessionId
	 * @param request
	 * @param key      session的关键字：LOGIN_SESSION_ID   |   OLD_LOGIN_SESSION_ID
	 * @return
	 */
	...
	public static boolean isLoginTimeout(Session session){
		if(session!=null){
			return System.currentTimeMillis()-session.getOperateDate().getTime()>SESSION_TIMEOUT_MILLISECOND;
		}
		return false;
	}
	...
	public static void removeSession(Session session){
		if(session!=null){
            SESSION_SERVICE.deleteSessionByUUID(session.getUuid());
            removeSessionFromList(session.getUuid());
		}
	}
	/**
	 * 清除某个用户的session
...
//			List<Session> list = new ArrayList<Session>(SESSION_LIST.values());
...
//							|| (session.getUser().msg().toLowerCase().contains(search)
